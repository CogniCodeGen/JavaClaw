...
public interface ExtensionSettingsGateway {
     ...
     * @return 命令结果
     */
    CompletableFuture<ExtensionRpcContracts.CallResult> execute(String extensionId, ViewCommandInvocation invocation);
     ...
     */
    CompletableFuture<AttachmentRef> upload(ViewAttachmentUploadRequest request);

    /**
     * 订阅当前 Workspace 中一个 Extension 的资源失效事件。
     *
     * @param extensionId 精确 Extension 标识
     * @param listener 只接收标识与当前 Workspace 匹配的事件
     * @return 幂等取消句柄
     */
    ...
    static ExtensionSettingsGateway disconnected() {
        return new ExtensionSettingsGateway() {
            ...
            private <T> CompletableFuture<T> failed() {
                return CompletableFuture.failedFuture(new IllegalStateException("Desktop 尚未连接 App Server"));
            }
        };
    }
}
