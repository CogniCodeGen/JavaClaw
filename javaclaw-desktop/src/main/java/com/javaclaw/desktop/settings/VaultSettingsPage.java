...
public final class VaultSettingsPage implements ManagedSettingsPage {
    ...
    private final VBox content = components.page("Secret Vault");
    ...
    public VaultSettingsPage(CoreSettingsGateway gateway) {
        ...
        resetZone = new DangerZone(
                "永久 Reset Vault",
                "永久清除所有 Secret 并使旧 CredentialRef 失效。输入 RESET VAULT 后才可执行。",
                "永久 Reset Vault",
                this::resetVault);
        buildLayout();
    ...
    private void buildLayout() {
        Label hint = new Label("Secret 只能写入、轮换、清除和查看“已配置”元数据；任何页面都不能读取、复制或导出明文。");
        hint.setWrapText(true);
        hint.getStyleClass().add("sec-hint");
        FormSection status =
                new FormSection("运行状态", "VAULT_LOCKED 时管理中心仍可打开，但 Provider、MCP、Site 和 Schedule 必须 fail closed。");
        ...
        FormSection masterKey = new FormSection("主密钥管理", "轮换和 reset 必须由 App Server 完成原子提交与危险确认，Desktop 不直接访问系统凭据设施。");
        Label rotationHint = new Label("轮换会重新加密全部记录，但不会改变 CredentialRef 或业务 revision。");
        ...
        resetConfirmation.setAccessibleText("Vault reset 危险确认");
        FormSection reset = new FormSection("危险确认", "确认文本不可本地化或模糊匹配，避免误触导致全部 Secret 失效。");
    ...
    private void render(VaultSettingsState snapshot) {
        ...
        receipt.setText(snapshot.receipt()
                .map(value -> value.action() + " · " + value.affectedCredentialCount() + " 条 · " + value.completedAt())
                .orElse("—"));
        updateActionAvailability(snapshot);
    }

    private void renderStatus(VaultStatus status) {
        state.setText(status.state() == VaultState.READY ? "READY" : "VAULT_LOCKED");
        reason.setText(status.reason().name());
