...
| 能力 | 说明 |
|------|------|
| 🌐 浏览器自动化 | 内置 Playwright：导航 / 截图标注 / 页面交互 / Cookie / JS 执行 / PDF 导出；BrowserContext 按聊天会话隔离，支持同站点多账号选择与自动登录 |
...
- **显式纠错** —— 说一句「不是 X，而是 Y」即刻生效：本轮同步撤销旧记忆并写入更正，后续对话优先采用；已被否定的说法不会再被自动写回记忆。只有把新旧说法都讲清楚才算纠错，「报错了」这类反馈不会误改记忆；记错了可在记忆中心撤销或删除
...
- **记忆中心** —— 九个分区的可视化管理面板：
  ...
  - **纠错** 查看你给出的更正记录，可撤销或删除；受影响的事实一键恢复
