...
需要继续使用已有数据时，应使用匹配该数据库的程序版本或经过验证的迁移方案，不能手动改写
`CORE.SCHEMA_HISTORY` 或跳过 checksum 校验。需要重新建立开发环境时，先停止 App Server 和 Desktop，
完整备份 `data-v6`（包括数据库、Vault 和 Blob），再让当前程序初始化空的 `data-v6`；
新库需要重新配置 Provider 等设置，旧数据保留在备份中。
