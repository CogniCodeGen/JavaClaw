...
public final class RouterPrompts {
    ...
    public static final String ROUTING_PROMPT_HEADER = """
            ...
            - coding：代码编写、Bug 分析、代码审查、架构设计、算法讲解；工程文件的读取/编辑、代码内容检索(grep)、按模式查找文件(glob)
            - knowledge：编程以外的知识问答、概念分析、方案对比；知识库管理（导入/检索/删除文档）
            - web：网页浏览、信息搜索、表单填写、页面操作、截图；站点凭据登记/查询、保存登录会话、切换账号
            ...
            - media：图片查看（在弹窗中展示图片给用户）、图片/PDF 的 OCR 文字识别（提取图片或 PDF 中的文字）
            - mcp：MCP 外部工具调用；MCP Server 配置的新增、更新、启停、删除
            """;
    ...
    public static final String ROUTING_PROMPT_FOOTER = """
            ...
            9. 站点/网站凭据、登录账号、保存站点会话或“添加到站点管理” → toolGroups 必须包含 web
            10. 新增、配置、启停、删除 MCP Server → toolGroups 必须包含 mcp
