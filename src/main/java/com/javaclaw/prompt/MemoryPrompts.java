...
public final class MemoryPrompts {
    ...
    public static final String DISTILL_PROMPT = """
            ...
            2. 用户对输出格式、文风、工具选择、流程方式提出的**明确要求**（如"给我 markdown 格式的"、"用表格展示"），视为偏好记录，措辞归一化为陈述句（如"用户偏好文档输出为 markdown 格式"）。
            3. 不记录一次性问答内容、临时任务详情、对话过程本身。
            4. 输出 Markdown 列表，每条以 `- ` 开头，每条一行简短陈述。
            5. 如果没有值得记录的事实，**只输出"无"**这一个字。
            6. 中文输出。
