...
final class ServicePluginWire {
    ...
    record Frame(
        ...
        static Frame control(Type type, long generation, String pluginId, String pluginVersion,
                             JsonNode payload) {
            return new Frame(PROTOCOL_MAJOR, PROTOCOL_MINOR, type, "", generation,
                    pluginId, pluginVersion, "", "", 0, 0,
                    "application/json", payload, false, "", "");
        }
    ...
    record Configure(
        ...
        Configure {
            ...
            resources = resources == null ? new Resources(256, 0, 1, 64) : resources;
    ...
    record Endpoint(
            String id, String protocol, String bindAddress, int port, boolean tlsEnabled,
    ...
    static final class Codec implements AutoCloseable {
        ...
        Frame read() throws IOException {
            ...
            if (length <= 0 || length > MAX_FRAME_BYTES) {
                throw new IOException("invalid service-plugin frame length: " + length);
            }
            byte[] value = input.readNBytes(length);
            if (value.length != length) throw new EOFException("truncated service-plugin frame");
            Frame frame = json.readValue(value, Frame.class);
            if (frame.protocolMajor() != PROTOCOL_MAJOR) {
                throw new IOException("incompatible service-plugin protocol");
            }
            return frame;
        }

        synchronized void write(Frame frame) throws IOException {
            byte[] value = json.writeValueAsBytes(frame);
            if (value.length == 0 || value.length > MAX_FRAME_BYTES) {
                throw new IOException("service-plugin frame exceeds limit");
            }
