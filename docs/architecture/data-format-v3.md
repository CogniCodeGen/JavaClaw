...
准备操作幂等；失败不会删除、改名或迁移目录中的任何内容。非空旧格式 `data/` 因缺少
正确标记而被拒绝，不会被 3.0 自动改写。
...
- 升级：先备份并移走旧格式 `data/`，再让 3.0 创建新的 `data/`；配置和插件按 3.0 规则重新创建或安装；
- 回滚：恢复事先备份的旧格式目录；旧版本不得直接打开 3.0 的 `data/`；
- 3.0 已产生的数据应单独保留，回滚过程不得删除或覆盖；
